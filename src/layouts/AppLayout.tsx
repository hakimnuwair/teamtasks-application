  ...
  return (
    <aside
      className={cn(
        ...
        // Glass styling
        "bg-white/80 dark:bg-[#161B22]/95",
        "backdrop-blur-xl border-r border-white/40 dark:border-[#21262D]/80",
        // Desktop width toggle
        "md:relative md:h-screen",
        sidebarOpen ? "md:w-60" : "md:w-[68px]",
        // Mobile: always full width in the overlay
        "w-72",
      )}
    >
      {/* ── Header ── */}
      <div className="h-16 flex items-center justify-between px-4 border-b border-black/5 dark:border-white/5 shrink-0">
        ...
        {/* Collapsed logo (desktop icon-only) */}
        {!sidebarOpen && (
        ...
        {/* Toggle btn desktop / close btn mobile */}
        <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? [
                          ...
                          "shadow-[0_0_12px_rgba(79,70,229,0.12)] dark:shadow-[0_0_12px_rgba(99,102,241,0.20)]",
                        ]
      ...
      <div className="p-3 border-t border-black/5 dark:border-white/5 shrink-0">
        {sidebarOpen || onMobileClose ? (
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              onClick={() => logout()}
              className="p-1.5 rounded-md text-[#94A3B8] hover:text-coral-500 hover:bg-coral-50 dark:hover:bg-coral-500/10 transition-all"
              title="Logout"
            >
          ...
          <button
            onClick={() => logout()}
            className="w-full flex items-center justify-center p-2 rounded-lg text-[#94A3B8] hover:text-coral-500 hover:bg-coral-50 dark:hover:bg-coral-500/10 transition-all"
            title="Logout"
          >
...
function Topbar({
  ...
  return (
    <header
      className={cn(
        "h-16 shrink-0 flex items-center justify-between px-4 md:px-6",
        "sticky top-0 z-40",
        // Glass
        "bg-white/75 dark:bg-[#0D1117]/88",
        "backdrop-blur-xl",
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          ...
          {unreadCount > 0 && (
            <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-coral-500 shadow-[0_0_6px_rgba(244,63,94,0.60)]" />
          )}
        </NavLink>
...
function BottomNav() {
  const { unreadCount } = useNotificationStore();
  const BOTTOM_ITEMS = NAV_ITEMS.slice(0, 5); // Dashboard, Reminders, Groups, Notifications, Activity
            ...
            {({ isActive }) => (
              <span className="flex flex-col items-center gap-1 py-1">
                <span className="relative">
                  ...
                  />
                  {isNotif && unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-coral-500 shadow-[0_0_6px_rgba(244,63,94,0.60)]" />
                  )}
                </span>
                <span
                  className={cn(
